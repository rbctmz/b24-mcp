...
- Ресурсы MCP для получения списков сделок, лидов, контактов, пользователей, задач, валют и других справочников Bitrix24
- Инструменты MCP для получения сущностей CRM (сделок, лидов, контактов, пользователей, задач) и компаний (`getCompanies`/`getCompany`)
...
- Обогащение лидов `_meta`-данными: ответственный, создатель, кто изменил, стадия, источник, валюта. Инструмент `getLeads` теперь использует тот же кэш, что и ресурс `crm/leads`, поэтому в `structuredContent` возвращаются `_meta`, `aggregates` (подсчёты по ответственным и статусам), а также `hints.copyableFilter` с примером недельного диапазона и сортировки, чтобы агент мог копировать фильтр для следующего шага, а warnings теперь рекомендуют именно диапазон «последняя неделя» (`>=DATE_CREATE`, `<=DATE_CREATE`).
- Ресурс `versions/releases` (синоним `releases`) возвращает историю релизов (`version`, `status`, `title`, `notes`), сформированную из `CHANGELOG.md`, чтобы агент мог получить список изменений прямо внутри MCP.
- Ресурс `versions/releases` может подтягивать релизы напрямую с GitHub, если задать `GITHUB_RELEASES_REPO` (аутентификация `GITHUB_TOKEN`, тайм-аут `GITHUB_TIMEOUT_SECONDS`, кеш `GITHUB_CACHE_TTL_SECONDS`). При недоступности GitHub возвращается локальный `CHANGELOG.md`.
- Предложения по диапазонам и подсказки строятся в таймзоне, заданной через `SERVER_TIMEZONE` (по умолчанию `UTC`), чтобы `YYYY-MM-DDTHH:MM:SS±HH:MM`-границы отражали локальный день портала.
- Настройка через переменные окружения (`.env`)
- `SERVER_TIMEZONE` (необязательно): IANA-таймзона (например, `Europe/Moscow`), которая используется для форматирования диапазонов дат в подсказках и предупреждениях.
...
- MCP-инструменты возвращают `structuredContent` с полным ответом Bitrix24 и предупреждениями. При отсутствии диапазона по дате любой из инструментов списка (`getDeals`, `getLeads`, `getContacts`, `getUsers`, `getTasks`, `getCompanies`) добавляет сообщение, рекомендуемые фильтры и `suggestedFix`, что позволяет fastmcp/SGR автоматически повторить запрос.
...
- Для списочных инструментов (`getLeads`, `getDeals`, и т.п.`) в `structuredContent.pagination` теперь возвращаются `limit`, `start`, `next`, `total` и `fetched`, чтобы можно было сразу ответить, сколько всего лидов выбрано, и, при необходимости, шаг за шагом получать все страницы.
- Справочники стадий лидов/сделок (`crm/lead_statuses`, `crm/deal_stages`) теперь возвращают `group` и человекочитаемое `groupName` (напр. «В работе», «Заключена», «Провалена») по семантике стадии, чтобы можно было агрегировать данные по этим группам не прибегая к дополнительной логике.
- Инструмент `getLeads` теперь принимает параметр `statusSemantics` (или `groupSemantics`), куда можно передать `["process"]`/`["success"]`/`["failure"]` и он автоматически заменит на список `STATUS_ID`, соответствующих этим группам.
- И добавлен `getLeadCalls`: он вызывает `crm.activity.list` (OWNER=1, TYPE=2), вызывает `crm.activity.get` и `voximplant.statistic.get` для каждой записи и возвращает список звонков с `CALL_ID`, длительностью и ссылкой на запись.

## Ресурс истории релизов

Запросите ресурс `versions/releases` (или alias `releases`) через `/mcp/resource/query`, чтобы получить список релизов из `CHANGELOG.md`. Каждая запись содержит `version`, `status`, `title` и `notes`, поэтому агент может понять, что поменялось:
...
Укажите `GITHUB_RELEASES_REPO=owner/repo`, чтобы отдавать аккуратную историю релизов прямо с GitHub. При необходимости добавьте `GITHUB_TOKEN` (для приватного репозитория или чтобы избежать лимитов), настройте `GITHUB_TIMEOUT_SECONDS` и `GITHUB_CACHE_TTL_SECONDS`. Если GitHub недоступен, MCP возвращает локальные записи из `CHANGELOG.md`.
...
| `/mcp/tool/call` | POST | Выполнение инструмента (`getDeals`, `getLeads`, `getContacts`, `getUsers`, `getTasks`, `getCompanies`, `getCompany`) |

Запросы к `crm/deals`, `crm/leads`, `crm/contacts` и `crm/tasks` по умолчанию возвращают не более пяти записей при пустом фильтре, чтобы без явной просьбы не отдавать весь список.

### Отладочное логирование

- Чтобы видеть входящие JSON-RPC запросы и результаты инструментов прямо в логах сервера (помогает понять, какие фильтры нужны, например, для Open WebUI), перед запуском установите переменную `SERVER_LOG_LEVEL=debug` (можно в `.env`). При DEBUG-уровне в логах появятся детальные параметры `tool`/`resource` и информация о пагинации.
